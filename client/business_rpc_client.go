...
func InitBusinessRpcClientConn() (*grpc.ClientConn, error) {
	...
	conn, err := grpc.NewClient(Config.ServerConfig.RpcBusinessHost, opts...)
	if err != nil {
		return nil, err
	}
	return conn, nil
...
func InvokeBusinessRPCMethod(ctx context.Context, methodName string, ao *business_rpc.RequestAO) (*business_rpc.ResponseVO, error) {
	conn, err := InitBusinessRpcClientConn()
	if err != nil {
		return nil, err
	}
	defer conn.Close()
	client := business_rpc.NewBusinessRPCClient(conn)
	...
	if result[0].IsValid() {
		vo, ok := result[0].Interface().(*business_rpc.ResponseVO)
		if !ok {
