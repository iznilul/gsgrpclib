...
func InitTrackRpcClientConn() (*grpc.ClientConn, error) {
	...
	conn, err := grpc.NewClient(Config.ServerConfig.RpcTrackHost, opts...)
	if err != nil {
...
func InvokeTrackRPCMethod(ctx context.Context, methodName string, ao *track_rpc.RequestAO) (*track_rpc.ResponseVO, error) {
	conn, err := InitTrackRpcClientConn()
	if err != nil {
		return nil, err
	}
	defer conn.Close()
	client := track_rpc.NewTrackRPCClient(conn)
	...
	if result[0].IsValid() {
		vo, ok := result[0].Interface().(*track_rpc.ResponseVO)
		if !ok {
